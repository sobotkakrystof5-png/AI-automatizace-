...
// propojené linkou v brand-electric (stejný akcent jako svislá linka v
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-5 top-1/2 h-px -translate-y-1/2 overflow-hidden">
        <div className="animate-flow-pulse absolute top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-brand-electric shadow-[0_0_8px_2px_var(--color-brand-electric)]" />
      </div>
      ...
      {icons.map((Icon, index) => (
        ...
        >
          <Icon className="h-5 w-5 text-brand-electric" />
        </div>
      ))}
    </div>
  );
